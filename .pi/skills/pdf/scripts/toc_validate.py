...
HINT_INDICATORS = ['', 'Tip:', 'Update Field', '']
        ...
        if child.tag == '{http://schemas.openxmlformats.org/wordprocessingml/2006/main}sdt':
            ...
            for instr in child.findall('.//' + _w('instrText')):
                ...
                if sdtPr is not None:
                    ...
                    if alias is not None and alias.get(_w('val'), '').upper() in ('TOC', ''):
                        indices.append(idx)
                        continue
...
def fix_docx(docx_path: str, output_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    pi = 0
    caption_filter = re.compile(r'^[]\s*\d')
    ...
    toc_title = "  " if content_lang == 'zh' else "Table of Contents"
            ...
            if pPr is not None:
                ...
                if pStyle is not None:
                    ...
                    if is_any_heading_style(style_val):
                        ...
                        if text and level > 0:
                            # Skip table/figure captions (e.g. " 1：xxx", " 2：xxx")
        ...
        if child.tag == qn('w:sdt'):
            ...
            if not is_toc_sdt:
                ...
                if sdtPr is not None:
                    ...
                    if alias is not None and alias.get(qn('w:val'), '').upper() in ('TOC', ''):
                        is_toc_sdt = True
...
def fix_docx_accurate_pages(fixed_docx_path: str, pass1_pdf_path: str, output_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    headings = []
    caption_pattern = re.compile(r'^[]\s*\d')
