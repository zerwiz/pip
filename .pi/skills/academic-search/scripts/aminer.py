...
pip gateway's /v1/functions/invoke endpoint.
...
REQUEST_TIMEOUT = 60
CONFIG_PATHS = [
    pathlib.Path.cwd() / ".pi-config",
    pathlib.Path.home() / ".pi-config",
    pathlib.Path("/etc/.pi-config"),
]
...
def load_config() -> dict[str, Any]:
    for path in CONFIG_PATHS:
        ...
        if cfg.get("baseUrl") and cfg.get("apiKey"):
            ...
            return cfg
    raise SystemExit(
        "pi config not found. Create .pi-config in cwd, $HOME, or /etc "
...
def invoke(config: dict[str, Any], function_name: str, arguments: dict[str, Any]) -> Any:
    ...
    try:
        ...
        if exc.code == 401 and "X-Token" in detail:
            hint = " (hint: add a valid 'token' field to .pi-config)"
        elif exc.code == 403:
            hint = " (hint: request rejected by auth — check 'apiKey' / X-PI-From)"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Call one of 27 AMiner Open Platform APIs via the pi gateway.",
