...
def check_font_fallback(root: ET.Element) -> CheckResult:
    ...
    SAFE_FONTS = {
        ...
        "", "FangSong", "FangSong_GB2312", "", "KaiTi",
        # English
...
def check_toc(root: ET.Element, docx_path: str = "") -> CheckResult:
    ...
    # Check 1: Document has a "" / "  " / "Table of Contents" title but no TOC field
    has_toc_title = False
    toc_title_pattern = re.compile(r'^(?:\s*|table\s+of\s+contents|contents)$', re.IGNORECASE)
    for p in paragraphs:
        if p.tag != f"{{{w_ns}}}p":
            continue
