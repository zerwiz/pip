...
const TillDoneParams = Type.Object({
  ...
  texts: Type.Optional(
    Type.Array(Type.String(), {
      description:
        "Multiple task texts (for add). Use this to batch-add several tasks at once.",
    }),
  ),
...
export default function (pi: ExtensionAPI) {
  ...
  const refreshUI = (ctx: ExtensionContext) => {
    if (tasks.length === 0) {
      ...
      ctx.ui.setStatus(
        `${label}: ${tasks.length} tasks (${remaining} remaining)`,
        "tilldone",
      );
    }
  ...
  pi.on("agent_end", async (_event, _ctx) => {
    ...
    const taskList = incomplete
      .map(
        (t) =>
          `  ${STATUS_ICON[t.status]} #${t.id} [${STATUS_LABEL[t.status]}]: ${t.text}`,
      )
      .join("\n");
  ...
  pi.registerTool({
          ...
          if (!params.text) {
            return {
              content: [
                {
                  type: "text" as const,
                  text: "Error: text (title) required for new-list",
                },
              ],
            ...
            if (!confirmed) {
              return {
                content: [
                  {
                    type: "text" as const,
                    text: "New list cancelled by user.",
                  },
                ],
          ...
          if (params.id === undefined) {
            return {
              content: [
                {
                  type: "text" as const,
                  text: "Error: id required for remove",
                },
              ],
        ...
        case "update": {
          if (params.id === undefined) {
            return {
              content: [
                {
                  type: "text" as const,
                  text: "Error: id required for update",
                },
              ],
          ...
          if (!params.text) {
            return {
              content: [
                {
                  type: "text" as const,
                  text: "Error: text required for update",
                },
              ],
          ...
          const result = {
            content: [
              {
                type: "text" as const,
                text: `Updated #${toUpdate.id}: "${oldText}" → "${toUpdate.text}"`,
              },
            ],
        ...
        case "clear": {
          if (tasks.length > 0) {
            ...
            if (!confirmed) {
              return {
                content: [
                  { type: "text" as const, text: "Clear cancelled by user." },
                ],
          ...
          const result = {
            content: [
              { type: "text" as const, text: `Cleared ${count} task(s)` },
            ],
    ...
    renderCall(args, theme) {
      ...
      if (args.texts?.length)
        text += ` ${theme.fg("dim", `${args.texts.length} tasks`)}`;
    ...
    renderResult(result, { expanded }, theme) {
      ...
      switch (details.action) {
        case "new-list": {
          let msg =
            theme.fg("success", "✓ New list ") +
            theme.fg("accent", `"${details.listTitle}"`);
        ...
        case "list": {
          ...
          if (details.listTitle) {
            listText +=
              theme.fg("accent", details.listTitle) + theme.fg("dim", "  ");
          }
        ...
        case "toggle": {
          ...
          const msg = text?.type === "text" ? text.text : "";
          return new Text(
            theme.fg("accent", "⟳ ") + theme.fg("muted", msg),
            0,
            0,
          );
        }
        ...
        case "remove": {
          ...
          const msg = text?.type === "text" ? text.text : "";
          return new Text(
            theme.fg("warning", "✕ ") + theme.fg("muted", msg),
            0,
            0,
          );
        }
